...
<cfset Report_read = FindNoCase(".RTF", curLine, 1)><!--- The above - may have caused this not to work. --->
   ...
   <!--- +#Report_Read#+ -- +#Schedules_read#+ ---> 
   
   <cfif Report_read gt 0 and Schedules_read eq 0>
	   <!--- NGL::06/29/05::Catch !!!!!!!sasy!!!!!!! in the current line --->  
<!--- GET /Sasy/ 2005-06-27 10:05:14 159.142.4.53 GET /SASY/2005/0624-ARBDAY-04. ----- 2005-06-27 - 2005- 0624--A-04-  --->	   
	   ...
	   <!--- NGL::06/29/05::If we read sassy in the line --->
	   <cfif read gt 0>
		...
		<cfelse>
			...
			<!--- insert into sasy_logreader
			(date_viewed, yearof, dateEntered, office, category, filename)
			values('#date_viewed#', '#yearof#', '01-01-1111', 'Null', 'Null', '#filename#')
			<br> --->
