...
<cfset Report_read = FindNoCase(".RTF", curLine, 1)><!--- The above - may have caused this not to work. --->
   ...
   <cfif Report_read gt 0 and Schedules_read eq 0>
	   ...
	   <cfif read gt 0>
		...
		<cfif len(#dateEntered#) LT 8><!--- most if not all dates were a 2 character item so had to set a default date --->
			 <cfset dateEntered = "01-01-" & #yearof#><!---  --->
			 <cfset dateEntered = #DateFormat(dateEntered, "mm/dd/yyyy")#>
			 <!---<cfset dateEntered = ""> --->#dateEntered#~
		<cfelse><!--- most if not all dates were a 2 character item if not, then used real date. --->
