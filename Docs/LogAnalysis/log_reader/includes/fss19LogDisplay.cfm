...
<cfquery datasource="OnlineReportsSQL" name="findInfo"><!--- ReptFileName, CategoryCode, ReportName --->
	...
	where CatagoryCode = '#form.RTFCatItems#'
</cfquery> 
...
<cfloop query="findInfo"  startrow="#start#" endrow="#end#">
	<!---<cfquery datasource="OnlineReportsSQL" name="findInfo2004"><!--- timeout="20000" datepart("yyyy", date_viewed) --->
		...
		and filename = '#ReptFileName#'
	</cfquery>  --->
	
	<!--- 
		<cfstoredproc procedure="getFSS19Data" datasource="OnlineReportsSQL">      
			<cfprocparam type="In" cfsqltype="CF_SQL_VARCHAR" dbvarname="DateInfo" value="#form.DateInfo#" null="No"> 
			<cfprocparam type="In" cfsqltype="CF_SQL_VARCHAR" dbvarname="ReptFileName" value="#ReptFileName#" null="No">      
			<cfprocresult name="theResults" resultset="1">
 		</cfstoredproc>
		<cfquery datasource="OnlineReportsSQL" name="findInfo2004">
			execute procedurename '#form.DateInfo#','#ReptFileName#' -- 2006, 1800
		</cfquery>
	 --->
	 	<cfquery datasource="OnlineReportsSQL" name="findInfo2004">
			execute findInfo2004 '#form.DateInfo#','#ReptFileName#'
		</cfquery><!---  --->
	...
	<tr bgcolor="###iif(currentrow MOD 2,DE('F8F8F8'),DE('eeeeee'))#">
		<td align="center" class="content">#CatagoryCode# </td>
