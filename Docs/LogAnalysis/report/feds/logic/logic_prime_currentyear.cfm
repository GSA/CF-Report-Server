...
<cfloop index="doy_id" from="1" to="#dim#">
		...
		<cfif #ParameterExists(client.ReptYear)# IS "YES" AND #ParameterExists(client.ReptRegion)# IS "YES">
			<cfparam name="session.directory" default="#RootDir# & '#ReptYear#' & '\' & '#ReptRegion#' & '\fss23' & '\' & '#ReptDate#' & '\'">
			<cfparam name="session.linkurl" default="#RootURL# & '#ReptYear#' & '/' & '#ReptRegion#' & '/' & 'fss23/' &  '#ReptDate#' & '/'">
			
			<cfset session.directory = #RootDir# & "#ReptYear#" & "\" & "#ReptRegion#" & "\fss23" & "\" & "#ReptDate#" & "\">
			<cfset session.linkurl = #RootURL# & "#ReptYear#" & "/" & "#ReptRegion#" & "/" & "fss23/" &  "#ReptDate#" & "/">
		</cfif>
