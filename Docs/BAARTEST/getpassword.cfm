...
function topwindow()
 {
msgWindow=window.open('<cfoutput>#session.LinkURL##session.FileName2#.TXT</cfoutput>' ,'JavaScriptIt','toolbar= 0,location= 0,directories= 0,status= 0,menubar= 0,scrollbars= 0,resizable= 1,copyhistory= 0,width=150,height=300');
 }
// End the hiding here.-->
</SCRIPT>
...
session.FileName2 = #session.FileName2#<br />
</cfoutput>
<cfabort>


	<CFDIRECTORY action="list" directory="#session.directory#" name="contents" sort="name" filter="*.rtf,*.TXT">

	<cfquery name="getreport" datasource="OnLineReports">
		select	* from Baartestreportname
		where catagorycode = 'B'
			AND reptfilename = '#session.FileName2#'
