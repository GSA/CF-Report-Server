...
<body>
		...
		<cfquery name="getData" datasource="Remarketing">
			Select 
				...
				Onlinesales.locid,
				<!--- CarInfo.Online as CarInfoOnline,	flag for pending.available, etc --->
			...
			WHERE
				...
				<!--- AND	AuctionSchedule.Online is null --->
		</cfquery>
...
<CFIF (getData.CarInfoOnline NEQ "3") AND (getData.CarInfoOnline NEQ "4")>
	<CFFILE action="APPEND" file="D:\reports\docs\otherftp\FleetAuctions\testAutoAuctions_extract.txt" variable="csv" output="#myline#">
<CFELSE>
...
<CFFILE action="rename" source="D:\reports\docs\otherftp\FleetAuctions\testAutoAuctions_extract.txt"
 destination="D:\reports\docs\otherftp\FleetAuctions\testAutoAuctions_extract_#dateformat(now(),"yyyymmdd")#.txt"
 attributes="Normal">
