...
<table align="center" width="520" border="1" cellspacing="0" cellpadding="6" nowrap bordercolor="#808080" cols="4">
	...
	<cfif year GE 2009> <!--- new way --->
		...
		<TR VALIGN="BASELINE">
			...
			<cfoutput>
			<FONT SIZE="2"><a href="#rooturl##year#/CloseingSchedules/FSS19-#year#-PROCESSING.doc" target="_new">Closing Schedule</a> For #Monthname#</FONT></td>		
			</cfoutput>
		</tr>	
	<cfelse> 	<!--- old way --->
		<cfset monthname = '#MonthasString(month2)#'>
	...
	<CFIF #Contents.RecordCount# is not 0>
		<CFLOOP query="contents">
			...
			<CFIF Right(FileName,5) EQ "_WRAP">
				<!--- if filename ends in _WRAP then we look for the file info for the file equivalent without the _WRAP in the name --->
				<CFSET wrapFileName = ReplaceNoCase("#FileName#", "_WRAP", "")>
				<cfquery name="getreport" datasource="OnLineReports">
					...
					<cfif #session.reportcatagory# is 'S3'> <!--- S3 and SD have the same report names --->
						where catagorycode = 'SD'	AND reptfilename = '#wrapFileName#'
					<cfelse>
						where catagorycode = '#session.reportcatagory#'	AND reptfilename = '#wrapFileName#'
					</cfif>
				</cfquery>
			<CFELSE>
				<cfquery name="getreport" datasource="OnLineReports">
